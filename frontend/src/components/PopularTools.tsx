...
const PopularTools: React.FC<PopularToolsProps> = ({ tools }) => {
  const categories = [
    { name: '모든 도구', targetId: 'all-tools', icon: Grid3X3 },
    { name: 'PDF변환도구', targetId: 'pdf-tools', icon: FileText },
    { name: '이미지도구', targetId: 'image-tools', icon: Image },
    { name: 'AI도구', featured: true, targetId: 'ai-tools', icon: Brain },
    { name: '이미지변환도구', targetId: 'image-conversion-tools', icon: RefreshCw },
    { name: '문서도구', targetId: 'document-tools', icon: FileType },
    { name: 'YOUTUBE도구', targetId: 'youtube-tools', icon: Youtube },
    { name: '동영상도구', targetId: 'video-tools', icon: Video },
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((category) => {
            ...
            return (
              <div
                key={category.name}
                onClick={() => scrollToSection(category.targetId)}
