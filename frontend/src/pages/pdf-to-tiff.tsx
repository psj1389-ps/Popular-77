...
const PdfToBmpPage: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    setProgress(1);
    setProgressText("PDF를 TIFF로 변환 중...");
    setErrorMessage('');
    setShowSuccessMessage(false);
    setConversionProgress(0);
    ...
    try {
      ...
      const poll = async () => {
        ...
        if (j.status === "done") {
          ...
          if (!/\.(zip|tif|tiff)$/i.test(name)) {
            name = isZip ? `${name}.zip` : `${name}.tiff`;
          }
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-20 px-4 text-center relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
            ...
            <p className="text-lg opacity-90 max-w-2xl mx-auto">AI 기반 PDF to TIFF 변환 서비스로 문서를 TIFF 이미지 파일로 쉽게 변환하세요.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500">TIFF 이미지 파일 변환</p>
          </div>
