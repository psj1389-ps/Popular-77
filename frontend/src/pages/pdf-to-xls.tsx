...
const PdfToXlsPage: React.FC = () => {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (useSync) {
        ...
        const shown = await directPostAndDownload(`${API_DIRECT_BASE}/convert`, form, `${base}.xlsx`);
      ...
      timerRef.current = window.setInterval(async () => {
        ...
        if (j.status === "done") {
          ...
          setIsLoading(false);
          setShowSuccessMessage(true);
          setSuccessMessage(`변환 완료! 파일명: ${base}.xlsx로 다운로드됩니다.`);
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-20 px-4 text-center relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
            <div className="flex justify-center items-center gap-4 mb-4">
              ...
              <h1 className="text-4xl font-bold">PDF → XLS 변환기</h1>
            </div>
            <p className="text-lg opacity-90 max-w-2xl mx-auto">AI 기반 PDF to XLS 변환 서비스로 문서를 스프레드시트 파일로 쉽게 변환하세요.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-semibold text-gray-800">PDF → XLS 변환기</h2>
            <p className="text-gray-500">스프레드시트 파일 변환</p>
          </div>
      ...
      {/* PDF를 XLS로 변환하는 방법 가이드 섹션 */}
      <div className="bg-gray-50 py-16">
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-800 mb-4">PDF를 XLS로 변환하는 방법</h2>
            <p className="text-gray-600">간단한 4단계로 PDF를 스프레드시트 파일로 변환하세요</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow">
              ...
              <p className="text-gray-600 text-sm text-center">"변환하기" 버튼을 클릭하세요. AI 기반 엔진이 문서를 XLS 파일로 변환합니다.</p>
            </div>
            ...
            <div className="bg-white rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow">
              ...
              <h3 className="text-lg font-semibold text-gray-800 mb-2 text-center">XLS 파일 다운로드</h3>
              <p className="text-gray-600 text-sm text-center">변환이 완료되면, 스프레드시트(.xls) 파일을 즉시 다운로드할 수 있습니다.</p>
            </div>
          </div>
        </div>
      </div>
