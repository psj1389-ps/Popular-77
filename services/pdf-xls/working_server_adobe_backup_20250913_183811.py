...
def add_image_with_adobe_text(doc, image, section, adobe_blocks):
    """Adobe 좌표를 사용하여 정확한 텍스트 배치로 이미지와 텍스트 오버레이"""
    # 배경 이미지 추가 (기존과 동일)
    ...
    # Adobe 좌표 기반 텍스트 배치
    if adobe_blocks:
        print(f"  🎯 Adobe 좌표로 {len(adobe_blocks)}개 텍스트 블록 배치")
        
        # PDF 좌표를 DOCX 페이지 좌표로 스케일링
        pdf_width = image.size[0] / dpi  # PDF 원본 크기 (인치)
        pdf_height = image.size[1] / dpi
        
        # 스케일 비율 계산
        scale_x = fit_w / pdf_width if pdf_width > 0 else 1
        scale_y = fit_h / pdf_height if pdf_height > 0 else 1
        
        # Y 좌표 기준으로 정렬하여 줄별로 그룹화
        sorted_blocks = sorted(adobe_blocks, key=lambda b: (b['top'], b['left']))
        
        # 줄별로 그룹화 (Y 좌표 기준 임계값 사용)
        lines = []
        current_line = []
        last_y = None
        ...
        for block in sorted_blocks:
            y = block['top'] * scale_y
            if last_y is None or abs(y - last_y) < 0.2:  # 0.2인치 이내면 같은 줄
        ...
        print("  ⚠️ Adobe 블록이 없어 텍스트 오버레이 생략")
...
def add_image_and_overlay_text(doc, image, section):
    """배경 이미지를 추가하고 추출 텍스트를 원본 위치 근사로 덮어쓰기.
    주의: python-docx는 절대 좌표 배치가 제한적이라 근사 배치(줄 단위)로 구현합니다.
    원본과 동일한 시각적 결과에 최대한 가깝게 생성합니다."""
    # 본문 영역 크기 계산
    max_w_in = float(section.page_width.inches - (section.left_margin.inches + section.right_margin.inches))
    max_h_in = float(section.page_height.inches - (section.top_margin.inches + section.bottom_margin.inches))
    ...
    if not blocks:
        print("  - OCR 텍스트 없음: 이미지만 추가")
        return

    # 이미지와 페이지의 스케일 비율 계산
    # 문단을 위에서부터 순서대로 추가 (라인 그룹핑 후 왼->오 정렬)
    blocks.sort(key=lambda b: (b["y"], b["x"]))
    line_y = -1
    current_line = []
    lines = []
    for b in blocks:
        if line_y < 0 or abs(b["y"] - line_y) < 14:
    ...
    # 라인 간 세로 여백을 이미지 비율에 따라 조절
    for ln in lines:
        words = sorted(ln, key=lambda x: x["x"])
        ...
        run.font.name = "맑은 고딕"
        run.font.size = Pt(11)
        # 라인 높이를 원본 블록 평균 높이에 비례하도록 근사
        avg_h_px = max(12, int(sum(w["h"] for w in words) / max(1, len(words))))
        # 픽셀을 포인트로 대략 변환 (200dpi 가정, 1인치=72pt)
        line_pt = (avg_h_px / 200.0) * 72.0
        p.paragraph_format.space_after = Pt(max(0, line_pt * 0.1))
