...
def convert_pdf_to_gif():
    try:
                ...
                for item in data:
                    ...
                    if len(item) >= 3 and item[0] >= 245 and item[1] >= 245 and item[2] >= 245:
                        ...
                        if len(item) == 4:
                            ...
                            new_data.append(item + (255,))  # 알파 채널 추가
                img.putdata(new_data)
                
                # GIF용 P 모드로 변환 (투명도 유지)
        ...
        # 단일 이미지인 경우
        if len(images) == 1:
            print("DEBUG: 단일 페이지 처리 - GIF 다운로드")
            # 단일 페이지 처리
            # 원본 파일명을 직접 사용하도록 수정
            base_name = os.path.splitext(file.filename)[0]
            download_name = f"{base_name}.gif"
            ...
            image_buffer = io.BytesIO()
            if transparent_bg:
                processed_image.save(image_buffer, format='GIF', optimize=True, transparency=255)
            else:
                processed_image.save(image_buffer, format='GIF', optimize=True)
            image_buffer.seek(0)
        ...
        # 다중 페이지인 경우 ZIP으로 압축
            ...
            with zipfile.ZipFile(zip_io, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                for i, image in enumerate(images):
                    ...
                    img_io = io.BytesIO()
                    if transparent_bg:
                        processed_image.save(img_io, 'GIF', optimize=True, transparency=255)
                    else:
                        processed_image.save(img_io, 'GIF', optimize=True)
                    img_io.seek(0)
                    
                    # 원본 파일명을 직접 사용하도록 수정
                    base_name = os.path.splitext(file.filename)[0]
                    img_filename = f"{base_name}_page_{i+1}.gif"
            ...
            # 원본 파일명을 직접 사용하도록 수정
            base_name = os.path.splitext(file.filename)[0]
            zip_filename = f'{base_name}_gif.zip'
