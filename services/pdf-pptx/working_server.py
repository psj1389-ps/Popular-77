...
def convert_pdf_to_pptx_with_adobe(pdf_path, output_path):
    """Adobe PDF Services SDK 4.2를 사용하여 PDF를 PPTX로 직접 변환 (ExportPDFJob 사용)"""
    ...
    try:
        ...
        print(f"PDF → PPTX 변환: {pdf_path} → {output_path}")
        ...
        print(f"📤 Adobe SDK 4.2로 PDF->PPTX 변환 중... ({file_size / 1024:.1f}KB)")
        ...
        print(f"✅ Adobe SDK 4.2 변환 성공: {pdf_path} -> {output_path} (편집 가능한 PPTX)")
        return True
...
def pdf_to_docx(pdf_path, output_path):
    """PDF를 PPTX로 변환 - Adobe PDF Services SDK 4.2 무조건 우선 사용.
    ...
    try:
        ...
        if ADOBE_SDK_AVAILABLE:
            try:
                ...
                # Adobe ExportPDF API로 직접 PPTX 변환 (1회만 시도)
                adobe_success = convert_pdf_to_pptx_with_adobe(pdf_path, output_path)
                
                if adobe_success:
                    print(f"✅ Adobe SDK ExportPDF 성공: PDF를 편집 가능한 PPTX로 직접 변환 완료")
        ...
        # PPTX 파일 저장
        doc.save(output_path)
        print(f"✅ PDF → PPTX 변환 완료: {output_path}")
        return True
        
    except Exception as e:
        print(f"❌ PDF → PPTX 변환 오류: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def convert_file():
    try:
        ...
        try:
            ...
            if file_ext == 'pdf':
                # PDF → PPTX
                output_filename = 'file.pptx'
                output_path = os.path.join(OUTPUT_FOLDER, output_filename)
                print(f"PDF → PPTX 변환: {file_path} → {output_path}")
